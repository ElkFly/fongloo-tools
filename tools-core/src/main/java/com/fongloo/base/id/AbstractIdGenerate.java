 ...
 * 抽象ID生成器
 */
public abstract class AbstractIdGenerate<T extends Serializable> implements IdGenerate {

    /**
     * 最大17Bit的序列号
     * 二进制 11111111111111111 => 十进制 131071
     */
    private static final int MAX_ORDER_NO = 131071;

    /**
     * 时间戳的掩码是41Bit
     * 63Bit - 22Bit = 41Bit
     */
    private static final long TIME_CODE = Long.MAX_VALUE >>> 22;
     ...
     */
    private static final long START_TIME = 1420041600210L;

    /**
     * 机器码 (0-31)
     */
    private static final int MAX_MACHINE_CODE = 31;
        ...
        do {
            if (orderNo > MAX_ORDER_NO) {
                ...
                if (this.orderNo.compareAndSet(orderNo, 0)) {
                    // 这里使用cas操作，所以不需要加锁
                    // 1、操作失败了, 则表示别的线程已经更改了数据，则直接进行自增并获取则可以了
                    orderNo = 0;
                } else {
                    //注意：先增加再取值。
